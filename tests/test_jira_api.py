...
def test_jira_api_rejects_atlassian_api_server_configuration(tmp_path, monkeypatch):
    oauth_file = tmp_path / ".jira.oauth"
    oauth_file.write_text(json.dumps({
        'jira-auth-method': 'oauth',
    ...
    with pytest.raises(ValueError, match='Configure jira-server with the Jira site URL'):
        api.create_client()
